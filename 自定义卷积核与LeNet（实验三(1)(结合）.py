...
class LeNet(nn.Module):
    def __init__(self):
        ...
        self.conv3 = nn.Conv2d(1, 49, 5,padding=2)
        # self.conv4 = nn.Conv2d(49, 200, 5,padding=2)
        self.fc1 = nn.Sequential(
            nn.Linear(441,1500),  # 进行线性变换
            nn.ReLU()                    # 进行ReLu激活
        )
    ...
    def forward(self, x):
        ...
        x = self.conv2(x)
        x = F.relu(self.conv3(x))   # conv1 -> relu -> pooling -> conv2 -> pooling
        x = F.max_pool2d(x, 2)      # 池化，(2, 2) stride=2
        # x = F.relu(self.conv4(x))   # conv1 -> relu -> pooling -> conv2 -> pooling
        # x = F.max_pool2d(x, 2)      # 池化，(2, 2) stride=2
        x = x.view(x.size(0), -1)
        x = self.fc1(x)
        x = self.fc2(x)
